...
1. **선택 정렬:** 입력과 관계없이 선택 정렬이 항상 O(n²)인 이유는? 버블 정렬이나 삽입 정렬처럼 더 좋은 최선 사례를 가질 수 없는 이유는?
   ...
   > **정답:** 매 라운드마다 정렬되지 않은 부분의 극값을 찾으려면 **전체를 스캔** 해야 하므로, 입력이 이미 정렬되어 있어도 줄일 수 있는 비교가 없다. 점화식은 $T(n) = T(n-1) + \Theta(n)$이고, 이는 **최선/평균/최악 모두** $\Theta(n^2)$으로 풀린다. 버블 정렬은 한 패스에서 교환이 0회면 조기 종료할 수 있고 삽입 정렬은 `a[j] > key` 조건이 즉시 거짓이 되어 멈출 수 있지만, 선택 정렬에는 그런 신호가 없다 — 비교 횟수는 항상 정확히 $n(n-1)/2$이다.

2. **안정성:** 정렬의 안정성이 중요한 구체적인 예시를 들어라. (힌트: 스프레드시트를 두 개의 다른 열로 정렬하는 경우를 생각해 보라.)

   > **정답:** 학생 명단을 먼저 **이름**(가나다)으로 정렬한 후 **학점** 으로 다시 정렬하는 경우를 생각해 보자. **안정 정렬** 을 사용하면 같은 학점을 가진 학생들끼리는 이전 단계의 이름 순서가 그대로 유지되어, 학점별로 묶이면서 그 안에서는 이름순으로 정렬된 결과가 나온다. **불안정 정렬** 을 쓰면 두 번째 정렬이 같은 학점 내의 이름 순서를 흩뜨려 버린다. **기수 정렬** 도 이 성질에 결정적으로 의존한다 — 각 자릿수 정렬이 이전 자릿수의 순서를 보존해야 최종 결과가 올바르다.

3. **병합 정렬 vs 퀵 정렬:** 둘 다 평균 O(n log n)이지만, 실제로는 퀵 정렬이 일반적으로 더 빠르다. 왜 그런가? 퀵 정렬의 약점은 무엇인가?

   > **정답:** 퀵 정렬은 **제자리(in-place)** 로 동작하고 상수 인자가 작으며, partition이 메모리를 순차적으로 스캔하기 때문에 **캐시 지역성** 이 매우 우수하다. 반면 병합 정렬은 임시 배열을 할당하고 복사해야 하므로 $O(n)$의 추가 공간과 메모리 대역폭을 소비한다. 퀵 정렬의 **약점** 은 분할이 극단적으로 불균형할 때(예: 이미 정렬된 입력에서 첫/마지막 원소를 피벗으로 사용) $\Theta(n^2)$이 되는 최악의 경우이다. **랜덤 피벗**, **median-of-three**, 재귀가 너무 깊어지면 힙 정렬로 전환하는 **introsort** 등의 기법으로 이 최악 경우를 사실상 회피할 수 있다.

4. **파티션:** 배열 [3, 8, 2, 5, 1, 4, 7, 6]에서 피벗 = 6(마지막 원소)으로 CLRS 파티션 절차를 추적하라. 최종 배열 상태는?

   > **정답:** `pivot = 6`, 초기 `i = 0`. `j = 1..7` 스캔: `A[1]=3 ≤ 6` → i=1, A[1]↔A[1]; `A[2]=8 > 6` 건너뜀; `A[3]=2 ≤ 6` → i=2, A[2]↔A[3] → `[3,2,8,5,1,4,7,6]`; `A[4]=5 ≤ 6` → i=3, A[3]↔A[4] → `[3,2,5,8,1,4,7,6]`; `A[5]=1 ≤ 6` → i=4, A[4]↔A[5] → `[3,2,5,1,8,4,7,6]`; `A[6]=4 ≤ 6` → i=5, A[5]↔A[6] → `[3,2,5,1,4,8,7,6]`; `A[7]=7 > 6` 건너뜀. 마지막 A[i+1]↔A[r] = A[6]↔A[8] 교환 → **`[3, 2, 5, 1, 4, 6, 7, 8]`**, 피벗 위치 **6** 반환.

5. **힙 정렬:** 최대 힙을 구축하는 데 O(n log n)이 아닌 O(n)이 걸리는 이유는? (힌트: 대부분의 노드가 트리 하단에 있다.)

   > **정답:** 단순 분석으로는 $n$번 `heapify` × $O(\log n)$ = $O(n \log n)$로 보이지만, 이는 느슨한 상계이다. **`heapify`의 비용은 트리 전체 높이가 아니라 해당 노드의 높이에 비례** 하기 때문이다. 노드의 절반은 리프(높이 0, 작업 없음), 1/4는 높이 1, 1/8은 높이 2…와 같이 분포한다. 총 작업량은 $\sum_{h=0}^{\log n} \frac{n}{2^{h+1}} \cdot h$이고, $\sum h/2^h$ 급수는 상수로 수렴하므로 전체는 $\Theta(n)$이 된다. 대부분 노드는 거의 작업이 없고, 루트 근처의 소수 노드만 $O(\log n)$의 작업을 한다.

6. **하한:** 비교 기반 정렬 알고리즘이 O(n log n)보다 나을 수 없는 이유는? 결정 트리 모델이 알려주는 것은?

   > **정답:** 비교 기반 정렬은 각 비교 "A[i] ≤ A[j]?"가 내부 노드이고 각 리프가 $n!$개의 가능한 순열 중 하나인 **이진 결정 트리** 로 모델링할 수 있다. 알고리즘의 최악 비교 횟수는 이 트리의 **높이** 와 같다. 리프 $n!$개를 가진 이진 트리의 높이는 최소 $\lceil \log_2(n!) \rceil$이고, **Stirling 근사** 에 의해 $\log_2(n!) = \Theta(n \log n)$이다. 따라서 입력에 대해 비교만으로 정보를 얻는 어떤 알고리즘도 최악의 경우 $\Omega(n \log n)$번의 비교를 피할 수 없다.

7. **계수 정렬:** 계수 정렬이 값의 범위 k가 작아야 하는 이유는? k = n²이면 복잡도는 어떻게 되는가?

   > **정답:** 계수 정렬은 카운트 배열 `C[1..k]`를 할당하고 길이 $k$ 또는 $n$짜리 루프 세 개를 돌므로 시간 $\Theta(n + k)$, 공간 $\Theta(k)$이다. $k = O(n)$이면 둘 다 $\Theta(n)$으로 줄어 비교 기반 하한을 우회한다. 하지만 **$k = n^2$** 이면 복잡도가 $\Theta(n + n^2) = \Theta(n^2)$으로 병합 정렬보다 느려지고, 카운트 배열만으로 $\Theta(n^2)$의 메모리를 낭비한다. 그래서 계수 정렬은 값의 범위가 **작고 밀집되어 있을 때만** 유리하다.

8. **기수 정렬:** 기수 정렬이 LSD에서 MSD 순서로(MSD에서 LSD가 아닌) 처리해야 하는 이유는? 부분 정렬의 어떤 성질이 이를 가능하게 하는가?

   > **정답:** **LSD에서 MSD** 로 진행하면서 매 라운드에 **안정 정렬** 을 사용하면, $i$번째 자릿수로 정렬한 뒤에도 안정성 덕분에 같은 $i$번 자릿수를 가진 원소들 사이에서는 **이전 자릿수의 순서가 그대로 보존** 되어, 결과적으로 1부터 $i$까지의 자릿수 조합으로 정렬된 상태가 된다. MSD에서 LSD로 진행하면 그 반대 — 하위 자릿수 정렬이 이미 확립된 상위 자릿수 순서를 흩뜨려서, 버킷 단위의 복잡한 재귀가 필요해진다. 전체 방식은 내부 정렬(보통 **계수 정렬**)이 안정적이라는 성질에 의존한다.
