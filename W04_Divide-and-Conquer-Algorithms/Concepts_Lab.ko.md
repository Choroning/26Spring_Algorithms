...
> **선수 지식**: 4주차 이론 — 분할 정복, 병합 정렬, 퀵 정렬, 선택, 최근접 쌍. Python 3 설치. 재귀와 분할(partition) 절차에 대한 이해.
>
> **학습 목표**:
> 1. 병합 정렬을 구현하고 재귀 실행 트리를 추적할 수 있다
> 2. 분할 절차를 사용하여 랜덤 선택(k번째로 작은 원소)을 구현할 수 있다
> 3. 분할 정복을 이용하여 최근접 점 쌍 알고리즘을 구현할 수 있다
> 4. 단순 O(n^2) 접근법과 최적화된 분할 정복 해법을 실험적으로 비교할 수 있다
...
1. 병합 정렬 추적에서 8개 원소 배열에 대해 병합(merge) 함수는 몇 번 호출되는가? 이것은 재귀 트리와 어떤 관계가 있는가?

   > **정답:** 8개 원소의 경우 병합 함수는 **7번** 호출된다 — 재귀 트리의 각 **내부 노드** 마다 한 번씩이다. $n$개 원소의 병합 정렬은 $n$개의 리프(크기 1의 기저 사례)와 $n-1$개의 내부 노드를 갖는 균형 이진 트리를 만들고, 각 내부 노드가 정렬된 두 절반을 합치는 `MERGE` 호출 한 번에 대응한다. 일반적으로 $n$개 리프 → $n-1$번 병합이며, 레벨당 $\Theta(n)$의 병합 작업 × $\log_2 n$ 레벨 = $\Theta(n \log n)$이 된다.

2. 랜덤 선택이 매번 나쁜 피벗을 선택하면 최악 시간 복잡도는 얼마인가? 랜덤 피벗에서 이런 일이 발생할 확률은?

   > **정답:** 매번 최솟값이나 최댓값을 피벗으로 잡으면 분할 결과가 크기 $0$과 $n-1$이 되어 $T(n) = T(n-1) + \Theta(n) = \Theta(n^2)$이다. **균등 랜덤** 피벗 선택에서 각 레벨에서 극단값을 뽑을 확률은 $2/n$이고, **모든** 재귀 호출에서 연달아 그런 일이 일어날 확률은 대략 $(2/n)(2/(n-1)) \cdots \approx 2^n / n!$로 천문학적으로 작다. 기대 시간은 $\Theta(n)$이며 $\Theta(n^2)$ 최악의 경우는 랜덤 피벗 하에서는 실질적으로 발생하지 않는다.

3. 최근접 쌍 벤치마크에서 분할 정복 접근법이 완전 탐색보다 처음으로 빨라진 입력 크기는? 더 작은 크기에서는 왜 그렇지 않은가?

   > **정답:** 벤치마크 표 기준 **N = 100** 에서 이미 분할 정복이 1.5배 빠르고, N = 1,000에서 30배, N = 5,000에서 125배로 격차가 벌어진다. 매우 작은 $n$에서는 분할 정복의 **상수 인자** (재귀 호출 오버헤드, 띠를 y좌표로 정렬, 리스트 슬라이싱 등)가 점근적 이득을 잡아먹는다. $n$이 한 자릿수일 때는 완전 탐색의 단순한 $O(n^2)$ 이중 루프가 부가 비용이 있는 $O(n \log^2 n)$보다 빠르다 — 바로 이 때문에 기저 사례 `n <= 3`에서 완전 탐색으로 떨어뜨리는 것이다.

4. 스트립 검사에서 7개 이웃으로 제한하지 않고 모든 쌍을 비교하면 최근접 쌍 알고리즘의 복잡도는 어떻게 되는가?

   > **정답:** 최악의 경우 띠 안에 모든 $n$개의 점이 들어올 수 있으므로 — 예컨대 점들이 모두 분할선 근처에 몰린 경우 — 띠 내 모든 쌍을 비교하면 최상위 레벨에서만 $\Theta(n^2)$이 든다. 점화식은 $T(n) = 2T(n/2) + \Theta(n^2)$이 되고, 마스터 정리 **경우 3** 에 의해 $T(n) = \Theta(n^2)$이다 — 완전 탐색과 다를 바 없다. 기하학적 격자 논증에서 나오는 7개 이웃 상한이 바로 띠 검사를 $O(n)$으로 줄여 전체 $O(n \log^2 n)$을 보장하는 핵심이다.
