...
> **학습 목표**:
> 1. 분할 정복 패러다임(분할, 정복, 결합)을 설명하고 알고리즘에서 이를 식별할 수 있다
> 2. 마스터 정리를 적용하여 분할 정복 점화식의 시간 복잡도를 결정할 수 있다
> 3. 병합 정렬, 이진 탐색, 퀵 정렬, 선택 알고리즘을 단계별로 추적할 수 있다
> 4. 파티션 절차의 작동 방식을 설명하고, 퀵 정렬의 최선·평균·최악 경우를 분석할 수 있다
> 5. 중앙값의 중앙값 알고리즘과 이것이 O(n) 최악 선택을 보장하는 이유를 설명할 수 있다
> 6. 최근접 쌍 알고리즘을 적용하고 스트립 분석을 이해할 수 있다
> 7. 분할 정복이 부적절한 경우(중복 부분 문제)를 인식하고 DP를 대신 사용해야 할 때를 판단할 수 있다
...
52장의 카드가 섞여 있는 덱을 정렬해야 한다고 상상해 보자. 자연스러운 방법 중 하나는 덱을 절반으로 나누고, 각 절반을 정렬한 뒤, 두 절반을 합치는 것이다 — 이것이 바로 분할 정복 전략이다. 큰 문제를 작은 문제들로 쪼개는 이 패러다임은 컴퓨터 과학에서 가장 강력한 아이디어 중 하나로 꼽힌다.
...
> **결합의 구체적 예시:** 병합 정렬에서 결합은 두 정렬된 리스트를 합병하는 것($O(n)$)이다. 이진 탐색에서 결합은 자명하다(결과를 그대로 반환). 최근접 쌍 문제에서 결합은 주의 깊은 띠(strip) 분석이 필요하다. 결합 단계가 알고리즘의 전체 복잡도를 결정하는 경우가 많다.
...
|:-----|:-----|:-----------|
| 2 | 2 | 병합 정렬, 최근접 쌍 |
| 3 | 2 | 큰 정수 곱셈 (CLRS 4장에서 다룸; 본 과목에서는 다루지 않음) |
| 7 | 2 | Strassen 행렬 곱셈 (CLRS 4장에서 다룸; 본 과목에서는 다루지 않음) |
...
여기서 $\varepsilon > 0$은 임의의 양의 상수이다 — 이 조건은 $f(n)$이 $n^{\log_b a}$보다 단순히 약간 작은 것이 아니라 **다항식적으로** 작거나 크다는 것을 의미한다.

![재귀 트리 예시: T(n)=3T(n/4)+cn²](../images/ch04_p025_006.png)

*재귀 트리 예시: T(n)=3T(n/4)+cn²*

> **참고 (경우 3):** *정규 조건(regularity condition)* 도 필요하다: 어떤 $c < 1$에 대해 $a \cdot f(n/b) \le c \cdot f(n)$이 성립해야 한다.

**풀이 예시:** $T(n) = 3T(n/4) + cn^2$의 경우: $a=3$, $b=4$, $f(n)=cn^2$. $n^{\log_4 3} \approx n^{0.79}$을 계산한다. $f(n) = cn^2$이 $n^{0.79}$보다 다항식적으로 빠르게 증가하므로 (즉, $f(n) = \Omega(n^{\log_4 3 + \varepsilon})$, $\varepsilon \approx 1.21$), 이는 **경우 3** 에 해당하여 $T(n) = \Theta(n^2)$이다.
...
**병합 추적 — [27, 38]과 [3, 43]을 합병:**

| 단계 | 비교 | 동작 | 현재까지 결과 | i | j |
|------|------|------|-------------|---|---|
| 1 | L[1]=27 vs R[1]=3 | 3 < 27, R 선택 | [3] | 1 | 2 |
| 2 | L[1]=27 vs R[2]=43 | 27 < 43, L 선택 | [3, 27] | 2 | 2 |
| 3 | L[2]=38 vs R[2]=43 | 38 < 43, L 선택 | [3, 27, 38] | 3 | 2 |
| 4 | L[3]=∞ vs R[2]=43 | 43 < ∞, R 선택 | [3, 27, 38, 43] | 3 | 3 |

> **[자료구조]** 보초값(sentinel) ∞는 경계 검사의 필요성을 제거한다. 한 배열이 소진되면, 보초값 덕분에 나머지 배열의 원소들이 순서대로 복사된다.
...
**랜덤 분할(Randomized Partition):** $[p..r]$ 범위에서 임의의 인덱스 $r'$를 선택하고, $A[r']$와 $A[r]$을 교환한 뒤, 기존과 동일하게 PARTITION을 호출한다. 이 간단한 수정만으로 정렬된 또는 거의 정렬된 입력에서의 최악의 경우를 피할 수 있다.
...
단계 2: [31, 48, 20, 29, 65, 73]에서 재귀, 3번째로 작은 원소 찾기
  pivot=73으로 PARTITION
  결과: [31, 48, 20, 29, 65 | 73]
  피벗 위치 k=6

  i=3 < k=6 → 왼쪽 그룹 [31, 48, 20, 29, 65]에서 탐색

단계 3: [31, 48, 20, 29, 65]에서 재귀, 3번째로 작은 원소 찾기
  pivot=65로 PARTITION
  결과: [31, 48, 20, 29 | 65]
  피벗 위치 k=5

  i=3 < k=5 → 왼쪽 그룹 [31, 48, 20, 29]에서 탐색

단계 4: [31, 48, 20, 29]에서 재귀, 3번째로 작은 원소 찾기
  pivot=29로 PARTITION
  결과: [20 | 29 | 31, 48]
  피벗 위치 k=2

  i=3 > k=2 → 오른쪽 그룹 [31, 48]에서 (3-2)=1번째로 작은 원소 탐색

단계 5: [31, 48]에서 재귀, 1번째로 작은 원소 찾기
  pivot=48로 PARTITION
  결과: [31 | 48]
  피벗 위치 k=2

  i=1 < k=2 → 왼쪽 그룹 [31]에서 탐색
...
평균적으로, 랜덤 피벗은 원소들의 중간 50% (25번째 백분위수와 75번째 백분위수 사이)에 위치하므로, 더 큰 분할은 원래 크기의 최대 3n/4이다.
...
- 이로써 띠 검사가 점당 $O(1)$ 비교 (최대 7개 이웃)로 제한되어, 띠 전체에서 $O(n)$이 된다
...
지금까지 분할 정복의 다양한 성공적 응용을 살펴보았으니, 이제 이 패러다임이 실패하는 경우와 대안적 접근법(동적 프로그래밍)을 어떤 경우에 사용해야 하는지 살펴보자.
...
1. **분할 정복:** D&C의 세 단계는 무엇인가? "결합" 단계가 자명한 예와 비용이 큰 예를 각각 하나씩 들어라.

   > **정답:** 문제를 더 작은 부분 문제로 **분할(Divide)**, 각 부분 문제를 재귀적으로 **정복(Conquer)**, 부분 해를 합쳐 원래 문제의 해로 **결합(Combine)** 한다. **이진 탐색** 과 **퀵 정렬** 에서는 결합이 자명하다 — 이진 탐색은 결과를 그대로 반환하고, 퀵 정렬은 제자리 분할 덕분에 이미 정렬되어 있다. 반면 **병합 정렬** 에서는 결합이 비용이 크다 — 정렬된 두 절반을 합병하는 데 $\Theta(n)$이 들며, 이것이 전체 점화식 $T(n) = 2T(n/2) + \Theta(n)$의 핵심이다.

2. **마스터 정리:** 다음에 마스터 정리를 적용하라: (a) T(n) = 2T(n/2) + n, (b) T(n) = 4T(n/2) + n, (c) T(n) = T(n/2) + n². 각각의 경우와 결과를 기술하라.

   > **정답:** $f(n)$과 $n^{\log_b a}$을 비교한다. **(a)** $a=2, b=2$이므로 $n^{\log_2 2} = n$이고 $f(n) = n$ — **경우 2**, $T(n) = \Theta(n \log n)$ (병합 정렬). **(b)** $a=4, b=2$이므로 $n^{\log_2 4} = n^2$이고 $f(n) = n$이 다항식적으로 작다 — **경우 1**, $T(n) = \Theta(n^2)$. **(c)** $a=1, b=2$이므로 $n^{\log_2 1} = 1$이고 $f(n) = n^2$이 다항식적으로 크다 — **경우 3**, $T(n) = \Theta(n^2)$.

3. **병합 정렬:** 병합 정렬이 입력 순서에 관계없이 항상 Θ(n log n)인 이유는? 공간 복잡도는 얼마이며, 그 이유는?

   > **정답:** 병합 정렬은 원소 값과 무관하게 배열을 **무조건** 절반으로 나누고 합병한다 — 분할 단계도 결합 단계도 입력에 의존하지 않는다. 재귀 트리의 깊이가 $\log_2 n$이고 각 레벨에서 $\Theta(n)$의 합병 작업을 수행하므로, 최선·평균·최악 모두 $T(n) = \Theta(n \log n)$이다. **공간 복잡도** 는 $\Theta(n)$인데, `MERGE` 절차가 두 절반을 복사해 둘 보조 배열 `L`, `R`이 필요하기 때문이다 — 퀵 정렬의 제자리 분할 대비 병합 정렬의 가장 큰 단점이다.

4. **퀵 정렬 최악:** 마지막 원소를 피벗으로 선택할 때 O(n²)이 되는 구체적인 입력 배열을 제시하라. 랜덤 피벗 선택은 어떻게 도움이 되는가?

   > **정답:** `[1, 2, 3, 4, 5]` 같은 **이미 정렬된** (혹은 역순 정렬된) 배열이면 충분하다 — 피벗 `5`가 항상 최댓값이므로 분할 결과가 크기 $n-1$과 $0$으로 나뉘어 $T(n) = T(n-1) + \Theta(n) = \Theta(n^2)$이 된다. **랜덤 분할(Randomized Partition)** 은 임의 인덱스를 위치 `r`로 교환한 뒤 분할하므로, 적대적 입력이 피벗 선택과 상관관계를 갖지 못한다 — 입력 순서와 무관하게 **기대** 시간이 $\Theta(n \log n)$이 된다.

5. **선택:** 랜덤 선택의 기대 시간이 O(n log n)이 아닌 O(n)인 이유는? 먼저 정렬한 후 인덱싱하는 것과 어떻게 다른가?

   > **정답:** 선택은 분할 후 **원하는 순위가 있는 한쪽** 만 재귀하지만 퀵 정렬은 양쪽 모두 재귀한다. 랜덤 피벗이면 더 큰 분할의 기대 크기가 $\le 3n/4$이므로 점화식 $T(n) \le T(3n/4) + \Theta(n)$이 성립하고, 기하급수 $\sum (3/4)^k$로 합산하면 $4cn = \Theta(n)$이 된다. 정렬 후 인덱싱은 결국 묻지 않은 원소들의 순서까지 결정하는 낭비가 발생하므로 $\Theta(n \log n)$이다.

6. **중앙값의 중앙값:** 왜 특별히 5개씩 그룹으로 나누는가? 3개씩 나누면 어떻게 되는가?

   > **정답:** 그룹 크기 5는 점화식 $T(n) = T(n/5) + T(7n/10) + \Theta(n)$을 만들며, $1/5 + 7/10 = 9/10 < 1$이므로 작업량이 기하급수적으로 감소해 $\Theta(n)$으로 수렴한다. **3개씩** 묶으면 $T(n) = T(n/3) + T(2n/3) + \Theta(n)$이 되는데 $1/3 + 2/3 = 1$이라 기하급수적 감소가 성립하지 않고 점화식이 $\Theta(n \log n)$으로 풀려 선형 시간 보장을 잃는다. 5는 $<1$ 조건을 만족하는 가장 작은 홀수 그룹 크기이다.

7. **최근접 쌍:** 스트립 검사를 포인트당 최대 7개 이웃으로 제한할 수 있는 이유는? 이를 정당화하는 기하학적 논거는?

   > **정답:** 폭 $2d$의 띠 안에서 각 절반의 점들은 서로 거리 $\ge d$ 이상 떨어져 있다 — 만약 더 가까웠다면 재귀 호출에서 이미 발견했을 것이다. 점 $p$에 대해 거리 $d$ 미만인 점은 $p$ 아래쪽 $2d \times d$ 직사각형 안에만 존재할 수 있다. 격자 논증(packing argument)에 의해 이 직사각형 안에 서로 거리 $d$ 이상인 점은 **최대 8개** 까지만 들어갈 수 있으므로, 띠를 y좌표로 정렬한 뒤 $p$의 **다음 7개 이웃** 만 비교하면 충분하다.

8. **D&C vs DP:** 피보나치 수열을 D&C로 계산할 수 있다. 왜 이것이 나쁜 방법인가? 부분 문제 구조 측면에서 피보나치가 병합 정렬과 다른 점은?

   > **정답:** $F(n) = F(n-1) + F(n-2)$의 재귀는 **중복 부분 문제(overlapping subproblems)** 를 갖는다 — `F(n-2)`는 `F(n-1)` 안에서도 계산되고 `F(n)`의 직접 호출에서도 계산되며, 이런 중복이 지수적으로 누적되어 $\Theta(\varphi^n)$이 된다. 병합 정렬의 두 절반은 **서로 겹치지 않으므로** 어떤 원소도 중복 계산되지 않는다. 부분 문제가 겹칠 때는 **동적 프로그래밍(Dynamic Programming)** — 메모이제이션 또는 상향식 테이블 — 으로 각 값을 정확히 한 번만 계산하여 피보나치를 $\Theta(n)$으로 줄일 수 있다.
