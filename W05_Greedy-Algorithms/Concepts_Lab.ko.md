...
1. 동전 [1, 3, 4]와 금액 6에 대해 그리디 동전 거스름돈 알고리즘이 왜 실패하는가? 그리디가 항상 작동하려면 동전 단위에 어떤 속성이 있어야 하는가?
   ...
   > **정답:** 그리디는 가장 큰 동전을 먼저 선택한다: `4 → 1 → 1 = 3개`, 그러나 최적은 `3 + 3 = 2개`다. `4`를 선택하면 나머지 `2`가 한 개의 동전으로 만들어지지 않고 묶이는 반면, 작은 `3`을 선택하면 남은 금액과 정확히 맞아떨어진다. 그리디가 항상 작동하려면 화폐 단위에 **약수 속성** 이 있어야 한다 — 각 동전이 다음 작은 단위의 배수이거나 깔끔하게 정렬되어야 한다(예: {1, 5, 10, 25} 또는 {1, 10, 50, 100, 500}). 이 구조는 가장 큰 동전에 헌신해도 전역적으로 더 나은 조합을 차단하지 않음을 보장한다.

2. 물건을 쪼갤 수 없는 경우(0-1 배낭), 그리디 가치/무게 비율 전략이 왜 실패하는가? 구체적인 예를 들어라.

   > **정답:** 0-1 배낭에서는 단위당 가치가 높은 물건이 다른 물건의 분수로 채울 수 없는 **용량을 묶어둘** 수 있다. A(10, $60), B(20, $100), C(30, $120)과 용량 50에서: 그리디는 A 그리고 B를 선택 → $160, 20 lb가 비어 있다. 0-1 최적해는 **B + C = $220** 으로, 단위당 가치가 가장 높은 물건을 포기하고 용량을 완전히 활용한다. "포함할까 건너뛸까"는 이산적 선택이므로 그리디 선택 성질이 실패한다 — 분할 가능한 경우처럼 남은 공간을 C의 `2/3`로 채울 수 없다.

3. 허프만 코딩 예제에서 문자당 평균 비트 수가 이론적 엔트로피 하한에 얼마나 가까운가? 이 차이는 무엇을 의미하는가?

   > **정답:** "abracadabra"에 대해 허프만은 **2.091 비트/문자**를 달성하는 반면 엔트로피 하한은 **1.927 비트/문자** 이다 — 약 0.164 비트/문자 (~8.5%)의 차이가 있다. 이 차이는 허프만 코드가 각 기호에 **정수 비트 길이** 를 사용해야 하기 때문에 발생한다. 반면 섀넌 엔트로피는 분수 비트 할당을 가정한다. 허프만은 **접두사 없는 코드 중에서** 증명적으로 최적이지만 엔트로피에 근접하려면 분수 비트를 효과적으로 인코딩하는 **산술 부호화(arithmetic coding)** 같은 기법이 필요하다. 알파벳 크기가 커지고 빈도가 비대칭일수록 차이는 줄어든다.

4. 활동 선택에서 종료 시각이 아닌 시작 시각으로 정렬하면 어떻게 되는가? 그리디 알고리즘이 여전히 최적 해를 만들어내겠는가?

   > **정답:** **아니다.** 시작 시각으로 정렬하면 많은 짧고 호환 가능한 활동을 차단하는 긴 활동을 선택할 수 있다. 반례: 활동 `[0, 10), [1, 2), [3, 4), [5, 6)`. 가장 빠른 시작 우선은 `[0, 10)`을 선택하고 멈춘다 — **1개 활동**. 가장 빠른 종료 우선은 `[1, 2), [3, 4), [5, 6)`을 선택한다 — **3개 활동**. 가장 빠른 종료 휴리스틱이 작동하는 이유는 일찍 끝나면 이후 선택을 위한 **남은 시간이 최대** 가 되기 때문이다. 가장 빠른 시작은 선택한 활동이 임의로 길게 진행될 수 있으므로 그런 보장이 없다.

5. 동전 집합 {12, 9, 1}에서 금액 18을 만들 때, 그리디는 7개를, 최적은 2개를 사용한다. 왜 이 비율이 테스트 케이스 중 가장 나쁜지, 이 동전 집합이 그리디에 특히 불리한 이유를 설명하라.

   > **정답:** 그리디는 `12`를 먼저 선택하여 `6`을 남기는데, `9`가 더 이상 들어맞지 않으므로 1원짜리 6개로 떨어진다 → **7개**. 최적: `9 + 9 = 2개`, 정확히 **3.5배** 더 나쁘다. 이 비율이 가장 나쁜 이유는 **12와 9의 격차가 작아서** (12 대신 9를 선택해도 동전 *한 개* 만 절약된다) 그리디 이득이 미미하지만, **12를 선택하면 나머지에 9를 전혀 사용할 수 없게 되기** 때문이다. 화폐 단위가 서로 가까우면서도 약수 관계가 아니다 — 9와 1 사이에 대체할 동전이 없어, 묶인 나머지가 모두 1원짜리의 긴 꼬리로 무너진다.

6. 완전 탐색 배낭과 회의 스케줄러 모두 모든 부분집합을 검사한다. 왜 완전 탐색은 작은 N으로 제한되며, 그리디는 이를 어떻게 피하는가?

   > **정답:** 완전 탐색은 모든 `2^n` 부분집합을 열거한다 — N=20에서는 ~10⁶ (다룰 만함), N=30에서는 ~10⁹ (초~분 단위), N=50에서는 ~10¹⁵ (불가능). 이 지수적 폭증으로 인해 실제 입력에 대해서는 완전 탐색이 사용 불가능하다. **그리디** 는 **O(n log n)** (정렬이 지배적)에 실행되며 물건을 정확히 한 번만 순회하고, 부분집합을 열거하지 않는다. 매 단계에서 대안을 탐색하는 대신 단일 지역 최적 선택에 헌신함으로써 폭증을 피한다 — 완전 탐색의 보장된 정확성을 극적인 속도와 교환하며, 이는 그리디 선택 성질이 성립할 때 정당화된다.
