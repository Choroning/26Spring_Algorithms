...
> **선수 지식**: 2주차: 점근적 표기법과 복잡도 분석. 3주차: 정렬 알고리즘 (그리디 알고리즘의 서브루틴으로 사용). 4주차: 분할 정복 패러다임 (그리디와 비교를 위해). 기초 그래프 이론: 정점, 간선, 가중치 그래프, 트리, 사이클 (MST와 다익스트라 섹션).
>
> **학습 목표**:
> 1. 그리디 패러다임을 정의하고, 그리디 정확성의 두 조건(그리디 선택 성질, 최적 부분 구조)을 기술할 수 있다
> 2. 그리디가 작동하는 경우와 실패하는 경우를 구체적 반례와 함께 식별할 수 있다
> 3. 그리디 접근법으로 분할 가능 배낭 문제를 풀고, 0-1 배낭에서는 왜 실패하는지 설명할 수 있다
> 4. 활동 선택 알고리즘(가장 빠른 종료 시간)과 작업 스케줄링 알고리즘(가장 빠른 시작 시간)을 적용할 수 있다
> 5. 문자 빈도로부터 허프만 트리를 구축하고 압축률을 계산할 수 있다
> 6. 크루스칼과 프림 MST 알고리즘을 실행하고 차이점을 설명할 수 있다
> 7. 간선 이완을 사용하여 다익스트라 최단 경로 알고리즘을 실행하고, 음의 가중치에서 실패하는 이유를 설명할 수 있다
> 8. 그리디 알고리즘과 동적 프로그래밍을 비교하고, 주어진 문제에 어떤 접근법을 사용할지 결정할 수 있다
...
1. **그리디 선택 성질:** 그리디 선택 성질이 무엇인지 자신의 말로 설명하라. 이 성질이 성립하는 문제와 성립하지 않는 문제를 각각 하나씩 예를 들어라.

   > **정답:** **그리디 선택 성질** 은 매 단계에서 **지역적으로 최적인 선택** 을 함으로써 전역 최적해를 조립할 수 있다는 성질이다 — 앞을 내다보거나 다시 고려할 필요가 없다. 형식적으로, 적어도 하나의 최적해가 그리디 선택을 포함해야 하므로 그 선택에 헌신해도 최적성을 잃지 않는다. **성립하는 예**: 분할 가능 배낭 — 단위 무게당 가치가 가장 높은 물건을 최대한 가져가는 것이 항상 어떤 최적해의 일부이다. **실패하는 예**: 이진 트리 최대 경로 합 — 더 큰 자식(15 대신 36)을 선택하면 실제로 최적인 경로(작은 자식을 거치는)에 도달할 수 없다.

2. **동전 거스름돈:** 화폐 단위 {1, 5, 10, 25}에 대해 그리디가 최적임을 비형식적으로 증명하라. 그런 다음 12원짜리 동전을 추가하면 15원 만들기에서 그리디가 실패함을 보여라.

   > **정답:** {1, 5, 10, 25}에서는 각 단위가 더 작은 단위의 배수(또는 거의 배수)이므로, 큰 동전 하나를 더 작은 동전 여러 개로 바꾸면 개수가 같거나 늘어난다 — 예: 25원 하나 = 10원과 5원 조합 최소 3개. 따라서 그리디보다 큰 동전을 적게 쓰는 어떤 최적해도 개수를 늘리지 않고 그리디 형태로 재작성할 수 있다. **12원 동전을 추가한 경우** 15원 만들기: 그리디는 **12 + 1 + 1 + 1 = 4개**, 최적은 **10 + 5 = 2개**. 12원 동전이 약수 관계 구조를 깨뜨려 그리디 선택 성질이 무너진다.

3. **분할 가능 vs 0-1 배낭:** 물건 (무게, 가치): A(10, 60), B(20, 100), C(30, 120)과 용량 50이 주어졌을 때, 분할 가능 버전과 0-1 버전을 모두 풀어라. 답이 왜 다른가?

   > **정답:** 단위당 가치는 6, 5, 4 $/lb이다. **분할 가능**: A(10 lb, $60) + B(20 lb, $100) + C의 2/3(20 lb, $80) → **$240**. **0-1**: 그리디는 A + B = **$160**을 얻지만 20 lb를 낭비하고, 실제 0-1 최적해는 B + C = **$220**이다. 답이 다른 이유는 **0-1** 에서는 남은 용량을 단위당 가치가 낮은 물건의 일부로 채울 수 없기 때문이다. 따라서 단위당 가치가 높은 물건이 오히려 *함정* 이 되어 용량을 묶어둘 수 있다 — 바로 이 지점에서 그리디 선택 성질이 깨진다.

4. **활동 선택:** (시작, 종료) 활동들: (1,3), (2,5), (4,7), (1,8), (5,9), (8,10)에 가장 빠른 종료 시간 알고리즘을 적용하라. 몇 개의 활동이 선택되는가?

   > **정답:** 종료 시간 기준 정렬: (1,3), (2,5), (4,7), (1,8), (5,9), (8,10). (1,3) 선택 → `last_finish = 3`. (2,5)는 2 < 3이므로 건너뜀. (4,7)은 4 ≥ 3이므로 선택 → `last_finish = 7`. (1,8), (5,9)는 1, 5 < 7이므로 건너뜀. (8,10)은 8 ≥ 7이므로 선택 → `last_finish = 10`. **선택됨: {(1,3), (4,7), (8,10)} — 3개 활동.** 매 단계에서 가장 빨리 끝나는 호환 활동을 선택하면 이후 선택을 위한 남은 시간이 최대가 된다.

5. **허프만 코딩:** 빈도가 E=40, A=30, B=15, C=10, D=5인 문자들의 허프만 트리를 구축하라. 각 문자에 할당되는 코드는? 문자당 평균 비트 수는?

   > **정답:** 병합 순서: D(5)+C(10)=15; B(15)+15=30; A(30)+30=60; E(40)+60=100. 루트에서 왼쪽=0, 오른쪽=1로 읽으면: **E = `0`, A = `10`, B = `110`, C = `1111`, D = `1110`** (각각 1, 2, 3, 4, 4 비트). 총 비트 수 = 40·1 + 30·2 + 15·3 + 10·4 + 5·4 = 40+60+45+40+20 = **205 비트, 100 문자에 대해 평균 2.05 비트/문자**, 8비트 ASCII 기준선보다 훨씬 작다.

6. **크루스칼 vs 프림:** 6개 정점과 9개 간선이 있는 그래프에서 두 알고리즘을 모두 추적하라. 같은 MST를 만드는가? 다른 MST를 만들 수 있는 조건은?

   > **정답:** 두 알고리즘 모두 올바른 그리디 MST 알고리즘으로, **같은 총 가중치** 의 트리를 만든다. **모든 간선 가중치가 서로 다를 때** MST는 **유일** 하므로, 크루스칼과 프림은 구축 순서가 달라도(크루스칼은 포레스트를 전역적으로 합치고, 프림은 시작 정점에서 하나의 트리를 키운다) *정확히 같은* 간선 집합을 만든다. **간선 가중치에 동률** 이 있으면 여러 MST가 존재할 수 있다 — 두 알고리즘 모두 여전히 최소 가중치 트리를 찾지만, 동률 처리(그리고 프림의 시작 정점)에 따라 다른 간선이 선택될 수 있다.

7. **다익스트라:** 음의 간선이 있는 그래프에서 다익스트라가 잘못된 답을 내는 구체적인 예시를 구성하라. 그런 다음 위반되는 그리디 불변성을 설명하라.

   > **정답:** 정점 A, B, C와 간선 A→B = 2, A→C = 4, B→C = -3. 출발점 A. 다익스트라는 A를 확정(D=0)한 후, 확정되지 않은 최소 D 값을 선택한다 — `D[B] = 2 < D[C] = 4` — 그래서 **B를 확정**. 이후 D[C]가 2 + (-3) = -1로 갱신되어야 하지만, B가 이미 확정되어 다익스트라는 갱신을 시도해도 이미 늦었다. 즉 *나중에 확정될* 정점에서 음의 간선을 통해 *먼저 확정된* 정점까지의 경로가 더 짧을 수 있을 때 버그가 드러난다. **위반되는 불변성**: "vmin이 선택될 때 어떤 미래 경로도 더 짧을 수 없다" — 음이 아닌 가중치에서는 간선을 더해도 거리가 줄지 않지만, 음의 간선이 이 단조성을 깬다.

8. **그리디 vs DP 판단:** 각 문제에 대해 그리디 또는 DP를 결정하고 이유를 설명하라: (a) 임의 화폐 단위의 최소 동전, (b) 음이 아닌 가중치의 최단 경로, (c) 최장 공통 부분 수열, (d) 분할 가능 배낭.

   > **정답:** **(a) DP** — 임의 화폐 단위는 약수 구조를 깨뜨리므로(예: 금액 6에 대한 {1, 3, 4}), 그리디는 지역적으로 최선인 동전을 골라 나머지가 막힐 수 있다. `dp[i] = min(dp[i-c] + 1)`. **(b) 그리디** — 다익스트라 알고리즘. 음이 아닌 가중치가 "확정된 거리는 최적"이라는 불변성을 보존하기 때문이다. **(c) DP** — LCS는 부분 문제가 중첩되며 명확한 지역적 선택이 없다(어떤 문자 매치가 LCS에 속할지 미리 알 수 없다). 표준 `dp[i][j]` 테이블은 O(mn)이다. **(d) 그리디** — 단위 무게당 가치로 정렬한다. 분수가 허용되므로 모든 용량 단위를 가능한 최선의 비율로 사용할 수 있어 그리디 선택 성질이 성립한다.
